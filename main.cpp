...
const uint32_t PARTICLE_COUNT = 1000000;
...
private:
  ...
  void pickPhysicalDevice() {
    ...
    if (candidates.rbegin()->first > 0) {
      ...
      std::cout << "MSAA Level: " << msaaSamples << '\n';
    } else {
  ...
  int rateDeviceSuitability(VkPhysicalDevice device) {
    ...
    return score * (indices.isComplete() && extensionsSupported &&
  ...
  VkSampleCountFlagBits getMaxUsableSampleCount() {
    VkPhysicalDeviceProperties physicalDevieProperties;
  ...
  std::vector<uint32_t> getUniqueQueueFamilies() {
    ...
    std::vector<uint32_t> queueFamilies(uniqueQueueFamilies.begin(),
  ...
  void createGraphicsPipeline() {
    ...
    std::cout << "vert.spv: " << vertShaderCode.size() << " bytes\n";
    std::cout << "frag.spv: " << fragShaderCode.size() << " bytes\n";
  ...
  void transitionImageLayout(VkImage image, VkFormat format,
    ...
    VkCommandBuffer commandBuffer =
        beginSingleTimeCommands(graphicsCommandPool);
    ...
    endSingleTimeCommands(graphicsCommandPool, graphicsQueue, commandBuffer);
  }
  ...
  void endSingleTimeCommands(VkCommandPool commandPool, VkQueue commandQueue,
                             VkCommandBuffer commandBuffer) {
  ...
  void createCommandBuffers() {
    graphicsCommandBuffers.resize(MAX_FRAMES_IN_FLIGHT + 1);

    VkCommandBufferAllocateInfo graphicsBufferAllocInfo{};
    graphicsBufferAllocInfo.sType =
    ...
    graphicsBufferAllocInfo.commandBufferCount =
        (uint32_t)graphicsCommandBuffers.size();
    if (vkAllocateCommandBuffers(device, &graphicsBufferAllocInfo,
  ...
  void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    vkCmdDispatch(computeCommandBuffers[currentFrame], PARTICLE_COUNT / 256, 1,
                  1);
  ...
  void createShaderStorageBuffers() {
    ...
    for (auto &particle : particles) {
      ...
      float theta = rndDist(rndEngine) * 2 * 3.14;
  ...
  void createComputePipeline() {
    ...
    std::cout << "comp.spv: " << compShaderCode.size() << " bytes\n";
  ...
  void createComputeDescriptorSets() {
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      ...
      uniformBufferInfo.range = sizeof(DeltaTime);
  ...
  void createComputeCommandBuffers() {
    ...
    VkCommandBufferAllocateInfo computeBufferAllocInfo{};
    computeBufferAllocInfo.sType =
        VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    computeBufferAllocInfo.commandPool = computeCommandPool;
    computeBufferAllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    computeBufferAllocInfo.commandBufferCount =
        static_cast<uint32_t>(computeCommandBuffers.size());
    if (vkAllocateCommandBuffers(device, &computeBufferAllocInfo,
